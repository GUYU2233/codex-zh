...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "  ".into(),
                "使用 ChatGPT 登录，通过你的付费计划使用 Codex".into(),
            ]),
            Line::from(vec![
                "  ".into(),
                "也可以连接 API key，按实际用量计费".into(),
            ]),
            "".into(),
        ];
        ...
        let chatgpt_description = if !self.is_chatgpt_login_allowed() {
            ...
            "Plus、Pro、Business 和 Enterprise 计划均包含使用额度"
        };
        ...
        if !self.is_api_login_allowed() {
            lines.push(
                "  此 workspace 已禁用 API key 登录。请使用 ChatGPT 登录后继续。"
                    .dim()
                    .into(),
            );
            lines.push("".into());
    ...
    fn render_continue_in_browser(&self, area: Rect, buf: &mut Buffer) {
        ...
        {
            lines.push("  如果链接没有自动打开，请打开下面的链接完成认证：".into());
            lines.push("".into());
    ...
    fn render_chatgpt_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ 已使用你的 ChatGPT 账号登录".fg(Color::Green).into(),
            "".into(),
            "  开始之前：".into(),
            "".into(),
            "  请决定要授予 Codex 多大自主权限".into(),
            Line::from(vec![
                "  更多细节请查看 ".into(),
    ...
    fn render_api_key_entry(&self, area: Rect, buf: &mut Buffer, state: &ApiKeyInputState) {
        ...
        let mut intro_lines: Vec<Line> = vec![
            Line::from(vec![
                "> ".into(),
                "使用你自己的 OpenAI API key 按用量计费".bold(),
            ]),
            "".into(),
            "  在下方粘贴或输入你的 API key。它会保存在本地 auth.json 中。".into(),
            "".into(),
        ];
            ...
            if let SignInState::ApiKeyEntry(state) = &mut *guard {
                match key_event.code {
                    ...
                    KeyCode::Enter => {
                        ...
                        if trimmed.is_empty() {
                            self.set_error(Some("API key 不能为空".to_string()));
                            should_request_frame = true;
                        } else {
